#include "stm32l476.h"
#include "serial.h"
// Serial comms routine for the stm32l476 discovery board.
// Is uses USART2 as this is passed up to the host PC via
// the ST-Link's USB-UART emulator.
// Pins PD5 (tx) and PD6(rx) are used for transmission/reception.  This is AF7 
// in both cases.
...
void initUART(int BaudRate) {
	...
	ComOpen = 1;
	ComError = 0;
// Turn on the clock for GPIOD (usart 2 uses it) 
	RCC_AHB2ENR  |= BIT3;
...
// Configure the I/O pins.  Will use PD5 as TX and PD6 as RX so setup for alternate function
	GPIOD_MODER |= (BIT13 | BIT11);
	GPIOD_MODER &= ~BIT12;
	GPIOD_MODER &= ~BIT10;
// Turn on pull-ups to suppress noise when idle
    GPIOD_PUPDR |= (BIT12 | BIT10);
    GPIOD_PUPDR &= ~BIT13;
    GPIOD_PUPDR &= ~BIT11;
        
// The alternate function number for PD5 and PD6 is AF7 (see datasheet, reference manual)
	GPIOD_AFRL  |= (BIT26 | BIT25 | BIT24); 
	GPIOD_AFRL  &= ~BIT27;
	GPIOD_AFRL  |= (BIT22 | BIT21 | BIT20); 
	GPIOD_AFRL  &= ~BIT23;	
	
	
	BaudRateDivisor = 80000000;  // assuming 80MHz system clock
...
void drainTX(void)
{
    ...
    // Don't call this during interrupt context!
    while (USART2_CR1 & BIT3);
}
